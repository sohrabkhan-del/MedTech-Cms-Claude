...
export function MasterScanLogListPage() {
    ...
    () =>
      logs.filter((log) => {
        const productMatch =
          ...
          log.productName === appliedFilters.product
        const batchMatch =
          appliedFilters.batch === 'all' ||
          log.batchNumber === appliedFilters.batch
        const distributorMatch =
          appliedFilters.distributor === 'all' ||
          log.distributor === appliedFilters.distributor
        const dealerMatch =
          appliedFilters.dealer === 'all' ||
          log.dealer === appliedFilters.dealer
        const chemistMatch =
          appliedFilters.chemist === 'all' ||
          log.chemist === appliedFilters.chemist
        const barcodeMatch =
          !appliedFilters.barcodeNumber ||
          log.barcodeNumber
            .toLowerCase()
            .includes(appliedFilters.barcodeNumber.toLowerCase())
        const statusMatch =
          appliedFilters.scanStatus === 'all' ||
          log.scanResult === appliedFilters.scanStatus
        return (
          productMatch &&
          batchMatch &&
          distributorMatch &&
  ...
  const columns: CommonTableColumn<MasterScanLogEntry>[] = [
    ...
    },
    {
      ...
      render: (row) => (
        <Chip
          size="small"
          label={walletStatusConfig[row.walletStatus].label}
          color={walletStatusConfig[row.walletStatus].color}
        />
      ),
    },
    {
  ...
  const treeNodes = useMemo<HierarchyTreeNode[]>(() => {
    ...
    return Array.from(byProduct.entries()).map(([productName, productLogs]) => {
      ...
      const batchNodes: HierarchyTreeNode[] = Array.from(byBatch.entries()).map(
        ([batchNumber, batchLogs]) => {
          ...
          const distributorNodes: HierarchyTreeNode[] = Array.from(
            byDistributor.entries(),
          ).map(([distributor, distLogs]) => ({
                ...
                children: [
                  {
                    ...
                    onClick: () =>
                      navigate(`/audit/master-scan-table-logs/${log.id}`),
                  },
                  {
                    id: `${log.id}-reward`,
    ...
    () =>
      bubbleProducts.length === 0
        ...
        : filteredLogs.filter((log) =>
            bubbleProducts.includes(log.productName),
          ),
  ...
  const bubbleNodes = useMemo<BubbleGraphNode[]>(() => {
    ...
    const batchCounts = new Map<
      string,
      { label: string; count: number; productId: string }
    >()
    const distributorCounts = new Map<
      string,
      { label: string; count: number; batchId: string }
    >()
    const dealerCounts = new Map<
      string,
      { label: string; count: number; distributorId: string }
    >()
    const chemistCounts = new Map<
      string,
      { label: string; count: number; parentId: string }
    >()
    ...
    for (const log of bubbleLogs) {
      ...
      if (log.dealer) {
        ...
        dealerCounts.set(dealerId, {
          label: log.dealer,
          count: (dealerEntry?.count ?? 0) + 1,
          distributorId,
        })
        ...
        if (log.chemist) {
          ...
          chemistCounts.set(chemistId, {
            label: log.chemist,
            count: (chemistEntry?.count ?? 0) + 1,
            parentId: dealerId,
          })
        }
      } else if (log.chemist) {
    ...
    <>
      <Grid container spacing={3} sx={{ mb: 3 }}>
        ...
        <Grid size={{ xs: 12, sm: 6, lg: 2.4 }}>
          {isLoading ? (
            ...
            <StatCard
              label="Total Barcode Scans"
              value={scanKpis.totalBarcodeScans}
        ...
        <Grid size={{ xs: 12, sm: 6, lg: 2.4 }}>
          {isLoading ? (
            ...
            <StatCard
              ...
              value={scanKpis.successfulScans}
      ...
      {view === 'table' ? (
        <CommonTable
          ...
          searchKeys={(row) =>
            `${row.productName} ${row.productCode} ${row.batchNumber} ${row.barcodeNumber}`
          }
          ...
          actions={[
            {
              label: 'View',
              onClick: (row) =>
                navigate(`/audit/master-scan-table-logs/${row.id}`),
            },
          ]}
        ...
        <SectionCard title="Product Journey — Bubble Map">
          ...
          {bubbleProducts.length === 0 ? (
            ...
            <>
              <Typography
                sx={{ color: 'text.secondary', fontSize: '0.75rem', mb: 1.5 }}
              >
                Bubble size reflects record count at each level — Product →
                Batch → Distributor → Dealer → Chemist (chemist nests under the
                dealer that supplied it, or directly under the distributor if
        ...
        {(draft, setDraft) => (
          <Stack spacing={3}>
            ...
            <TextField
              ...
              onChange={(e) =>
                setDraft((prev) => ({ ...prev, distributor: e.target.value }))
              }
            >
            ...
            <TextField
              ...
              onChange={(e) =>
                setDraft((prev) => ({ ...prev, barcodeNumber: e.target.value }))
              }
            />
            <TextField
              select
              ...
              onChange={(e) =>
                setDraft((prev) => ({
                  ...prev,
                  scanStatus: e.target.value as ScanLogFilters['scanStatus'],
                }))
              }
            >
