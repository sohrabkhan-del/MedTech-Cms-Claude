...
const traceabilityStatusConfig: Record<
  ProductTraceabilityStatus,
...
export function FactoryBoxPage() {
  ...
  const { batchId, containerId, boxId } = useParams<{
    batchId: string
    containerId: string
    boxId: string
  }>()
  const { batch, container, box, isLoading } = useBoxDetail(
    batchId,
    containerId,
    boxId,
  )

  const [selectedProduct, setSelectedProduct] = useState<BoxProduct | null>(
    null,
  )
  ...
  const columns: CommonTableColumn<BoxProduct>[] = [
    ...
    },
    {
      ...
      render: (row) => (
        <Chip
          size="small"
          label={traceabilityStatusConfig[row.productStatus].label}
          color={traceabilityStatusConfig[row.productStatus].color}
        />
      ),
    },
    {
      ...
      header: 'Dealer Name',
      minWidth: 150,
      render: (row) => row.allocatedDealer,
    },
    {
      key: 'dealerAllocationDate',
      header: 'Dealer Assigned Date',
      minWidth: 150,
      render: (row) => row.dealerAllocationDate,
    },
    {
      key: 'allocatedChemist',
      header: 'Chemist Name',
      minWidth: 150,
      render: (row) => row.allocatedChemist,
    },
    {
      key: 'chemistAllocationDate',
      header: 'Chemist Assigned Date',
      minWidth: 150,
      render: (row) => row.chemistAllocationDate,
    },
    {
      key: 'currentHolder',
    ...
    },
    {
      ...
      minWidth: 130,
      render: (row) => row.redemptionStatus,
    },
    {
      ...
      render: (row) => (
        <Chip
          size="small"
          label={traceabilityStatusConfig[row.traceabilityStatus].label}
          color={traceabilityStatusConfig[row.traceabilityStatus].color}
        />
      ),
    },
  ]
    ...
    <>
      ...
      >
        ...
        <MuiLink
          ...
          onClick={() =>
            navigate(
              `/inventory/factory-inventory-upload/${batch.id}/${container.id}`,
            )
          }
        >
      ...
      <Stack spacing={3}>
        ...
        <SectionCard title="Traceability Table">
          <CommonTable
            ...
            searchKeys={(row) =>
              `${row.serialNumber} ${row.barcodeNumber} ${row.allocatedDealer} ${row.allocatedChemist}`
            }
            actions={[
              { label: 'View', onClick: (row) => setSelectedProduct(row) },
            ]}
