...
function representativeBatch(product: (typeof mockProducts)[number]): string {
  return (
    product.movementHistory[0]?.factoryUploadBatch ??
    `BATCH-${2026000 + Number(product.id.replace('product-', '')) * 3}`
  )
}
...
export const mockProductReports: ProductReportEntry[] =
  mockProducts.map(buildProductReport)

export function getProductReportById(
  id: string,
): ProductReportEntry | undefined {
...
export const productReportKpis = {
  ...
  rewardPointsIssued: mockProductReports.reduce(
    (sum, r) => sum + r.rewardPoints,
    0,
  ),
  activeProducts: mockProductReports.filter((r) => r.status === 'active')
    .length,
}

export const productReportCategoryOptions = productCategoryOptions
export const productReportBatchOptions = Array.from(
  new Set(mockProductReports.map((r) => r.batch)),
).sort()
