...
import { mockApprovalRequests, approvalRequestKpis } from '@/features/verification/mockApprovalRequests'
import type { ApprovalRequest, ApprovalStatus, RequestType } from '@/types/approvalRequest'
...
export function ApprovalRequestsListPage() {
  ...
  const resolveStatus = (request: ApprovalRequest) => statusOverrides[request.id] ?? request.status
  const topbarZone = region === 'All India' ? null : (region as PartnerZone)

  const filteredRequests = useMemo(
    () =>
      mockApprovalRequests.filter((request) => {
        const status = resolveStatus(request)
        const topbarRegionMatch = !topbarZone || request.region === topbarZone
        const statusMatch = appliedFilters.status === 'all' || status === appliedFilters.status
    ...
    [appliedFilters, statusOverrides, topbarZone],
  )
  ...
  const confirmDecision = () => {
    ...
    setStatusOverrides((prev) => ({ ...prev, [dialog.request!.id]: dialog.action === 'approve' ? 'approved' : 'rejected' }))
    closeDialog()
  }
