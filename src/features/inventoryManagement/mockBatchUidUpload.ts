...
function extractTrailingNumber(
  serial: string,
): { prefix: string; digits: number; width: number } | null {
...
export async function parseBmrFile(file: File): Promise<ParsedBmrResult> {
  ...
  const requiredFields: (keyof RawBmrRecord)[] = [
    'batchNumber',
    'startSerialNumber',
    'endSerialNumber',
  ]
  ...
  const rows: BmrBatchRow[] = dataRows
    ...
    .map((row, index) => {
      ...
      fieldForColumn.forEach((field, colIndex) => {
        ...
        record[field] = field.endsWith('Date')
      ...
      const isDuplicate =
        batchNumber !== '' && seenBatchNumbers.has(batchNumber)
  ...
  const summary: BmrValidationSummary = {
    ...
    duplicateBatches: rows.filter(
      (r) => r.validationNote === 'Duplicate batch number',
    ).length,
    invalidRanges: rows.filter(
      (r) =>
        r.validationNote === 'End serial precedes start serial' ||
        r.validationNote === 'Invalid serial number format',
    ).length,
    totalUidsGenerated,
  }
...
export interface ParsedMasterCartonResult {
  rows: MasterCartonLinkRow[]
  summary: MasterCartonUploadSummary
}

/**
 * Parses an uploaded Master Carton & Inner Box linkage file (UID, Master Carton Number
 * columns). Each UID is checked against the set generated by this session's BMR import.
 * Reference-only (Phase 1) until real line-side inner-box scanning is available.
 */
export async function parseMasterCartonFile(
  file: File,
  knownUids: Set<string>,
): Promise<ParsedMasterCartonResult> {
  const buffer = await file.arrayBuffer()
  const workbook = XLSX.read(buffer, { type: 'array', cellDates: false })
  const sheetName = workbook.SheetNames[0]
  if (!sheetName) throw new Error('The uploaded file has no sheets.')
  ...
  const headerRow = matrix[0]!.map((h) => normalizeHeader(String(h)))
  const uidCol = headerRow.findIndex((h) => h === 'uid')
  const cartonCol = headerRow.findIndex(
    (h) =>
      h === 'master carton number' ||
      h === 'master carton no.' ||
      h === 'master carton no',
  )

  if (uidCol === -1 || cartonCol === -1) {
    throw new Error(
      'The uploaded file is missing required column(s): UID, Master Carton Number.',
    )
  }
  ...
  const rows: MasterCartonLinkRow[] = dataRows
    .filter((row) => row.some((cell) => String(cell).trim() !== ''))
    .map((row, index) => {
      const uid = String(row[uidCol] ?? '').trim()
      const masterCartonNumber = String(row[cartonCol] ?? '').trim()

      const isDuplicate = uid !== '' && seenUids.has(uid)
      if (uid !== '') seenUids.add(uid)

      let validationNote: string | undefined
      if (!uid) validationNote = 'Missing UID'
      else if (!masterCartonNumber)
        validationNote = 'Missing Master Carton Number'
      else if (isDuplicate) validationNote = 'Duplicate UID'
      ...
      return {
        ...
        uid,
        masterCartonNumber,
        isValid: !validationNote,
        validationNote,
      }
    })

  const summary: MasterCartonUploadSummary = {
    totalRows: rows.length,
    validRows: rows.filter((r) => r.isValid).length,
    unknownUids: rows.filter(
      (r) => r.validationNote === 'UID not found in this BMR import',
    ).length,
    duplicateUids: rows.filter((r) => r.validationNote === 'Duplicate UID')
      .length,
  }
...
export function buildMappedBatches(batchRows: BmrBatchRow[]): MappedBatch[] {
  return batchRows
    ...
    .map((batch) => {
      ...
      const uidCount =
        start && end
          ? Math.min(end.digits - start.digits + 1, MAX_UIDS_PER_BATCH)
          : 0
      return {
        id: batch.id,
        batchNumber: batch.batchNumber,
