...
import { mockShowcaseProducts, showcaseProductKpis, showcaseCategoryOptions } from '@/features/marketing/mockShowcaseProducts'
...
export function ProductEnquiriesTab({ onViewProduct }: ProductEnquiriesTabProps) {
  ...
  const [respondedOverrides, setRespondedOverrides] = useState<Set<string>>(new Set())

  const allRows: EnquiryRow[] = useMemo(
    () => mockShowcaseProducts.flatMap((product) => product.enquiries.map((enquiry) => ({ ...enquiry, product }))),
    [],
  )
  ...
  const filteredRows = useMemo(
    () =>
      allRows.filter((row) => {
        ...
        const status = respondedOverrides.has(row.id) ? 'responded' : row.enquiryStatus
        const enquiryMatch = appliedFilters.enquiryStatus === 'all' || status === appliedFilters.enquiryStatus
    ...
    [allRows, appliedFilters, respondedOverrides, regionZone],
  )
  ...
  const columns: CommonTableColumn<EnquiryRow>[] = [
    ...
    { key: 'interestedDate', header: 'Interested Date', minWidth: 140, sortable: true, render: (row) => row.interestedDate },
    {
      ...
      render: (row) => {
        const status = respondedOverrides.has(row.id) ? 'responded' : row.enquiryStatus
        return <Chip size="small" label={enquiryStatusConfig[status].label} color={enquiryStatusConfig[status].color} />
      },
    },
    {
      key: 'deliveryStatus',
    ...
    <>
      ...
      <CommonTable
        ...
        actions={[
          ...
          { label: 'Mark as Responded', onClick: (row) => setRespondedOverrides((prev) => new Set(prev).add(row.id)) },
        ]}
