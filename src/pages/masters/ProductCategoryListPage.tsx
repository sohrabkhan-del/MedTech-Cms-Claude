...
import type { ProductCategory, ProductCategoryStatus } from '@/types/productCategory'
...
export function ProductCategoryListPage() {
  ...
  const columns: CommonTableColumn<ProductCategory>[] = [
    ...
    { key: 'categoryCode', header: 'Category Code', minWidth: 140, render: (row) => row.categoryCode },
    {
      ...
      minWidth: 160,
      render: (row) => getParentCategoryName(row.parentCategoryId) ?? '—',
    },
    {
      key: 'totalProducts',
    ...
    <>
      <Grid container spacing={3} sx={{ mb: 3 }}>
        <Grid size={{ xs: 12, sm: 6, lg: 3 }}>
          <StatCard label="Total Categories" value={productCategoryKpis.totalCategories} icon={<FolderTreeIcon size={20} />} iconColor="primary" />
        </Grid>
        <Grid size={{ xs: 12, sm: 6, lg: 3 }}>
          <StatCard label="Active" value={productCategoryKpis.activeCategories} icon={<CircleCheckIcon size={20} />} iconColor="success" />
        </Grid>
        <Grid size={{ xs: 12, sm: 6, lg: 3 }}>
          <StatCard label="Inactive" value={productCategoryKpis.inactiveCategories} icon={<BanIcon size={20} />} iconColor="error" />
        </Grid>
        <Grid size={{ xs: 12, sm: 6, lg: 3 }}>
          <StatCard label="Products Mapped" value={productCategoryKpis.totalProductsMapped} icon={<PackageIcon size={20} />} iconColor="secondary" />
        </Grid>
      </Grid>
