...
import { useGiftRuleForm } from '@/features/schemeManagement/hooks/useGiftRuleForm'
import { giftRuleFormDefaults, giftRuleFormSchema, type GiftRuleFormValues } from '@/features/schemeManagement/types/schemeManagement.types'
...
export function GiftRuleFormPage() {
  ...
  const { isEdit, rule, options, isLoading, isSubmitting, submit } = useGiftRuleForm(ruleId)

  const { control, handleSubmit, watch, reset } = useForm<GiftRuleFormValues>({
    resolver: zodResolver(giftRuleFormSchema),
    defaultValues: giftRuleFormDefaults,
  })
            ...
            {rewardTrack === 'Scheme Track' && (
              <Grid size={{ xs: 12, sm: 6 }}>
                ...
                <FormField name="schemeName" control={control} select {...fieldLabelProps}>
                  {(options?.schemeNameOptions ?? []).map((scheme) => (
