  ...
  if (scanResult === 'valid') {
    ...
    if (walletStatus === 'credited') {
      timeline.push({
        ...
        dateTime: dateTimeFromSeed(seed + 3),
      })
    } else if (walletStatus === 'failed') {
      timeline.push({
        id: `${entryId}-tl-3`,
        activity: 'Wallet Credit Failed',
        dateTime: dateTimeFromSeed(seed + 3),
      })
    }
    ...
    timeline.push({
      id: `${entryId}-tl-1`,
      activity: 'Duplicate Scan Detected',
      dateTime: dateTimeFromSeed(seed + 1),
    })
  } else {
    timeline.push({
      id: `${entryId}-tl-1`,
      activity: 'Invalid Barcode Flagged',
      dateTime: dateTimeFromSeed(seed + 1),
    })
  }
  return timeline
...
function buildScanReport(index: number): ScanReportEntry {
  ...
  const baseRewardPoints =
    scanResult === 'valid' ? seededNumber(seed, 1, 5) * 100 : 0
  const bonusPoints =
    scanResult === 'valid' && seed % 4 === 0
      ? seededNumber(seed + 1, 1, 3) * 100
      : 0
...
export const mockScanReports: ScanReportEntry[] = Array.from({
  length: 60,
}).map((_, index) => buildScanReport(index))
...
export const scanReportKpis = {
  ...
  successfulScans: mockScanReports.filter((s) => s.scanResult === 'valid')
    .length,
  failedScans: mockScanReports.filter(
    (s) => s.scanResult === 'duplicate' || s.scanResult === 'invalid',
  ).length,
  rewardPointsIssued: mockScanReports.reduce(
    (sum, s) => sum + s.rewardPoints,
    0,
  ),
}

export const scanReportProductOptions = Array.from(
  new Set(mockScanReports.map((s) => s.productName)),
).sort()
export const scanReportDealerOptions = Array.from(
  new Set(
    mockScanReports.filter((s) => s.dealerName).map((s) => s.dealerName!),
  ),
).sort()
export const scanReportChemistOptions = Array.from(
  new Set(
    mockScanReports.filter((s) => s.chemistName).map((s) => s.chemistName!),
  ),
).sort()
