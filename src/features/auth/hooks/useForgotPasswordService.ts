...
import { setPasswordResetRequest } from '@/features/auth/slices/authSlice'
...
export function useForgotPasswordService() {
  ...
  async function requestReset(email: string, phone?: string) {
    setIsLoading(true)
    setError(null)
    ...
    try {
      ...
      dispatch(setPasswordResetRequest({ email, phone }))
      navigate('/reset-otp')
    } catch (err) {
