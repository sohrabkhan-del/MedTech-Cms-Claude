...
import type { Scheme, SchemeAuditEntry, SchemeEligibleProduct, SchemeStatus } from '@/types/scheme'
...
export function SchemeDetailsPage() {
  ...
  const [statusOverride, setStatusOverride] = useState<SchemeStatus | null>(null)
  ...
  const currentStatus = statusOverride ?? scheme.status
    ...
    <>
      <Stack direction="row" sx={{ alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 2, mb: 3 }}>
        ...
        <Stack direction="row" spacing={1.5} sx={{ flexWrap: 'wrap' }}>
          ...
          <Button variant="outlined" color="error" startIcon={<Clock3 size={20} />} disabled={currentStatus === 'expired'} onClick={() => setStatusOverride('expired')} sx={{ fontSize: '0.75rem' }}>
            Expire
          </Button>
          <Button
            variant="outlined"
        ...
        <SectionCard title="Summary">
          <DetailFieldGrid
            fields={[
              ...
              { label: 'Status', value: <Chip size="small" label={statusConfig[currentStatus].label} color={statusConfig[currentStatus].color} /> },
            ]}
          />
        </SectionCard>
