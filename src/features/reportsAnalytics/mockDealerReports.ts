...
function findDealerWallet(dealerId: string) {
  return mockWallets.find(
    (wallet) => wallet.userType === 'Dealer' && wallet.userId === dealerId,
  )
}
...
function buildPerformanceSummary(
  dealer: (typeof mockDealers)[number],
): DealerReportPerformanceSummary {
  const validScans = dealer.scanHistory.filter(
    (s) => s.result === 'valid',
  ).length
  const scanToRewardConversion =
    dealer.scanHistory.length === 0
      ? 0
      : Math.round((validScans / dealer.scanHistory.length) * 100)
  const averageMonthlyScans = Math.round(dealer.totalScans / 12)
  const redemptionRate =
    dealer.totalScans === 0
      ? 0
      : Math.round((dealer.totalRedemptions / dealer.totalScans) * 1000) / 10
  const activeMonths = Math.min(
    12,
    Math.max(1, Math.round(dealer.totalScans / (averageMonthlyScans || 1))),
  )

  return {
    scanToRewardConversion,
    averageMonthlyScans,
    redemptionRate,
    activeMonths,
  }
...
function toReportRow(dealer: (typeof mockDealers)[number]): DealerReportRow {
  return {
    ...
    walletPoints: dealer.availablePoints,
    redemptions: dealer.totalRedemptions,
    status: dealer.status,
    onboardedDate:
      dealer.PointsHistory[0]?.date ?? dealer.scanHistory[0]?.scanDate ?? '—',
  }
}

export const mockDealerReports: DealerReportRow[] = mockDealers.map(toReportRow)

export function getDealerReportById(
  id: string,
): DealerReportDetails | undefined {
...
export const dealerReportKpis = {
  ...
  totalRewardPoints: mockDealerReports.reduce(
