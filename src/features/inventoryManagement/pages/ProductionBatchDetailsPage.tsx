...
const schemeStatusColor: Record<
  RelatedScheme['status'],
  'success' | 'info' | 'error'
> = {
  active: 'success',
  upcoming: 'info',
      ...
      <Stack spacing={3}>
        <SectionCard title="Summary">
          <DetailFieldGrid
            fields={[
              ...
              { label: 'Point Value', value: batch.PointValue },
              {
                label: 'Total Packages',
                value: batch.totalPackages.toLocaleString('en-IN'),
              },
              {
                label: 'Scan Status',
                value: `${batch.totalScans.toLocaleString('en-IN')} / ${batch.totalPackages.toLocaleString('en-IN')} scanned`,
              },
            ]}
          />
        </SectionCard>
        ...
        <SectionCard title="Scan Code / Barcode Information">
          <DetailFieldGrid
            fields={[
              {
                label: 'Barcode Series',
                value: batch.qrBarcodeInfo.barcodeSeries,
              },
              {
                label: 'Serial Number Range',
                value: `${batch.qrBarcodeInfo.serialRangeStart} – ${batch.qrBarcodeInfo.serialRangeEnd}`,
              },
              {
                label: 'Total Generated',
                value:
                  batch.qrBarcodeInfo.totalGenerated.toLocaleString('en-IN'),
              },
              {
                label: 'Total Available',
                value:
                  batch.qrBarcodeInfo.totalAvailable.toLocaleString('en-IN'),
              },
              {
                label: 'Total Scanned',
                value: batch.qrBarcodeInfo.totalScanned.toLocaleString('en-IN'),
              },
              {
                label: 'Duplicate Scans',
                value: batch.qrBarcodeInfo.duplicateScans,
              },
            ]}
          />
        </SectionCard>
        ...
        <SectionCard title="Distribution Journey">
          <CommonTable
            ...
            searchKeys={(row) =>
              `${row.distributor} ${row.dealer} ${row.chemist}`
            }
        ...
        <Grid container spacing={3}>
          <Grid size={{ xs: 12, sm: 6, lg: 3 }}>
            <StatCard
              ...
              value={batch.scanStatistics.totalSuccessfulScans.toLocaleString(
                'en-IN',
              )}
          ...
          <Grid size={{ xs: 12, sm: 6, lg: 3 }}>
            <StatCard
              label="Failed Scans"
              value={batch.scanStatistics.failedScans}
          ...
          <Grid size={{ xs: 12, sm: 6, lg: 3 }}>
            <StatCard
              label="Duplicate Scans"
              value={batch.scanStatistics.duplicateScans}
              icon={<ContentCopyOutlined size={20} />}
              iconColor="warning"
            />
          </Grid>
          <Grid size={{ xs: 12, sm: 6, lg: 3 }}>
            <StatCard
              ...
              value={batch.scanStatistics.geoFenceViolations}
              icon={<RoomOutlined size={20} />}
              iconColor="secondary"
            />
          </Grid>
        </Grid>
        ...
        <SectionCard title="Reward Summary">
          <DetailFieldGrid
            fields={[
              {
                label: 'Base Point Value',
                value: batch.rewardSummary.basePointValue,
              },
              { label: 'Bonus Points', value: batch.rewardSummary.bonusPoints },
              {
                label: 'Applied Scheme',
                value: batch.rewardSummary.appliedScheme,
              },
              {
                label: 'Total Reward Points Issued',
                value:
                  batch.rewardSummary.totalRewardPointsIssued.toLocaleString(
                    'en-IN',
                  ),
              },
            ]}
            ...
            fields={
              batch.uploadHistory[0]
                ? [
                    {
                      label: 'Upload File',
                      value: batch.uploadHistory[0].uploadFile,
                    },
                    {
                      label: 'Uploaded By',
                      value: batch.uploadHistory[0].uploadedBy,
                    },
                    {
                      label: 'Upload Date',
                      value: batch.uploadHistory[0].uploadDate,
                    },
                    {
                      label: 'Total Records',
                      value:
                        batch.uploadHistory[0].totalRecords.toLocaleString(
                          'en-IN',
                        ),
                    },
                    {
                      label: 'Success',
                      value:
                        batch.uploadHistory[0].success.toLocaleString('en-IN'),
                    },
        ...
        <SectionCard title="Fraud Detection">
          <DetailFieldGrid
            fields={[
              {
                label: 'Duplicate Scan Count',
                value: batch.fraudDetection.duplicateScanCount,
              },
              {
                label: 'Invalid Barcode Count',
                value: batch.fraudDetection.invalidBarcodeCount,
              },
              {
                label: 'Outside Geo-fence',
                value: batch.fraudDetection.outsideGeoFence,
              },
              {
                label: 'Suspicious Activity',
                value: batch.fraudDetection.suspiciousActivity,
              },
            ]}
          />
          {(batch.fraudDetection.suspiciousActivity > 0 ||
            batch.fraudDetection.outsideGeoFence > 0) && (
            <Stack
              direction="row"
              spacing={1}
              sx={{
            ...
            >
              ...
              <Typography sx={{ fontSize: '0.8125rem', fontWeight: 600 }}>
                This batch has flagged activity — review scan locations and
                duplicate attempts.
              </Typography>
            </Stack>
          )}
            ...
            fields={[
              {
                ...
                value:
                  batch.relatedRewards.totalRewardsGenerated.toLocaleString(
                    'en-IN',
                  ),
              },
              {
                ...
                value:
                  batch.relatedRewards.dealerRewards.toLocaleString('en-IN'),
              },
              {
                label: 'Chemist Rewards',
                value:
                  batch.relatedRewards.chemistRewards.toLocaleString('en-IN'),
              },
              {
                label: 'Redeemed Rewards',
                value:
                  batch.relatedRewards.redeemedRewards.toLocaleString('en-IN'),
              },
            ]}
          />
        </SectionCard>
