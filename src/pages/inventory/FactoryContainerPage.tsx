...
import { getBatchById, getContainerById } from '@/features/inventory/mockFactoryUploads'
import type { ContainerBox } from '@/types/factoryUpload'
  ...
  const boxColumns: CommonTableColumn<ContainerBox>[] = [
    {
      ...
      render: (row) => (
        <Typography
          ...
          onClick={() => navigate(`/inventory/factory-inventory-upload/${batch.id}/${container.id}/${row.id}`)}
        >
          {row.boxNumber}
        </Typography>
      ),
    },
    { key: 'productCount', header: 'Product Count', align: 'right', render: (row) => row.productCount },
  ...
  return (
    <>
      <Breadcrumbs separator={<NavigateNextOutlined sx={{ fontSize: 16 }} />} sx={{ mb: 1.5 }}>
        ...
        <Typography sx={{ fontSize: '0.8125rem', fontWeight: 600 }}>{container.containerNumber}</Typography>
      </Breadcrumbs>
        ...
        <SectionCard title="Summary">
          <DetailFieldGrid
            fields={[
              ...
              { label: 'Total Products', value: container.productCount.toLocaleString('en-IN') },
        ...
        <SectionCard title="Boxes">
          <CommonTable
            ...
            actions={[
              {
                label: 'View Box',
                onClick: (row) => navigate(`/inventory/factory-inventory-upload/${batch.id}/${container.id}/${row.id}`),
              },
            ]}
