...
import type { Gift, GiftStatus, StockStatus } from '@/features/schemeManagement/types/schemeManagement.types'

const stockStatusConfig: Record<StockStatus, { label: string; color: 'success' | 'warning' | 'error' }> = {
...
export function GiftCatalogueListPage() {
  ...
  useRegionTopbarHeader({
    ...
    subtitle: 'Manage all redeemable gifts available in the Rewards Marketplace.',
  })
  ...
  const giftCatalogueKpis = kpis ?? { totalGifts: 0, availableStock: 0, outOfStock: 0, totalRedemptions: 0 }

  const filteredGifts = gifts.filter((gift) => {
    const categoryMatch = appliedFilters.category === 'all' || gift.category === appliedFilters.category
    const brandMatch = appliedFilters.brand === 'all' || gift.brand === appliedFilters.brand
    const stockMatch = appliedFilters.stockStatus === 'all' || giftsService.getStockStatus(gift) === appliedFilters.stockStatus
    const statusMatch = appliedFilters.status === 'all' || gift.status === appliedFilters.status
    const minMatch = !appliedFilters.minCoins || gift.requiredCoins >= Number(appliedFilters.minCoins)
    const maxMatch = !appliedFilters.maxCoins || gift.requiredCoins <= Number(appliedFilters.maxCoins)
    return categoryMatch && brandMatch && stockMatch && statusMatch && minMatch && maxMatch
  })
  ...
  const columns: CommonTableColumn<Gift>[] = [
    {
      ...
      minWidth: 70,
      hideable: false,
      render: (row) => <Avatar src={row.giftImage} variant="rounded" sx={{ width: 36, height: 36 }} />,
    },
    { key: 'giftCode', header: 'Gift Code', minWidth: 130, render: (row) => row.giftCode },
    {
      key: 'giftName',
      header: 'Gift Name',
      minWidth: 190,
      ...
      render: (row) => (
        <Typography
          ...
          onClick={() => navigate(`/scheme-management/gift-catalogue/${row.id}`)}
        >
          {row.giftName}
        </Typography>
      ),
    },
    { key: 'category', header: 'Category', minWidth: 140, render: (row) => row.category },
    { key: 'brand', header: 'Brand', minWidth: 130, render: (row) => row.brand },
    {
      key: 'requiredCoins',
    ...
    },
    {
      ...
      render: (row) => (
        <Stack direction="row" spacing={1} sx={{ alignItems: 'center', justifyContent: 'flex-end' }}>
          <Typography sx={{ fontSize: '0.8125rem' }}>{row.availableQuantity}</Typography>
          <Chip size="small" label={stockStatusConfig[giftsService.getStockStatus(row)].label} color={stockStatusConfig[giftsService.getStockStatus(row)].color} />
        </Stack>
      ),
    },
    {
    ...
    },
    {
      ...
      header: 'Status',
      minWidth: 100,
      render: (row) => <Chip size="small" label={row.status === 'active' ? 'Active' : 'Inactive'} color={row.status === 'active' ? 'success' : 'default'} />,
    },
  ]
  ...
  return (
    <>
      ...
      <CommonTable
        ...
        onExportClick={() => {}}
        onImportClick={() => {}}
        createAction={{ label: 'Add Gift', to: '/scheme-management/gift-catalogue/new' }}
        defaultSortBy="giftName"
        actions={[
          { label: 'View', onClick: (row) => navigate(`/scheme-management/gift-catalogue/${row.id}`) },
          { label: 'Edit', onClick: (row) => navigate(`/scheme-management/gift-catalogue/${row.id}/edit`) },
        ...
        {(draft, setDraft) => (
          <Stack spacing={3}>
            ...
            <TextField
              ...
              onChange={(e) => setDraft((prev) => ({ ...prev, stockStatus: e.target.value as GiftFilters['stockStatus'] }))}
            >
            ...
            <TextField
              ...
              size="small"
              value={draft.status}
              onChange={(e) => setDraft((prev) => ({ ...prev, status: e.target.value as GiftFilters['status'] }))}
            >
            ...
            <TextField
              ...
              onChange={(e) => setDraft((prev) => ({ ...prev, minCoins: e.target.value }))}
            />
            <TextField
              type="number"
              label="Max Coins"
              ...
              onChange={(e) => setDraft((prev) => ({ ...prev, maxCoins: e.target.value }))}
            />
          </Stack>
        )}
      </FilterDrawer>
