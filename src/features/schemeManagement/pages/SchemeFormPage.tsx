...
import type { ApplicableUserType } from '@/features/schemeManagement/types/schemeManagement.types'
...
export function SchemeFormPage() {
  ...
  const { schemeId, category } = useParams<{
    schemeId: string
    category: string
  }>()
  ...
  const { control, handleSubmit, watch, reset } = useForm<SchemeFormValues>({
    ...
    defaultValues: {
      ...schemeFormDefaults,
      schemeCategory: categorySlug === 'sessional' ? 'seasonal' : 'general',
    },
  })
  ...
  useEffect(() => {
    ...
    reset({
      ...
      status:
        prefillSource.status === 'expired' || prefillSource.status === 'upcoming'
          ? 'active'
      ...
      <form onSubmit={onSubmit} noValidate>
          ...
          <Grid container spacing={2.5}>
            <Grid size={{ xs: 12, sm: 6 }}>
              ...
              <FormField
                name="schemeName"
                control={control}
                placeholder="e.g. Quarterly Scan Booster"
                {...fieldLabelProps}
              />
              {cloneSource && (
                <Typography
                ...
                >
                  Cloned from {cloneSource.schemeName} — update the name before
                  saving.
                </Typography>
              )}
            </Grid>
            ...
            <Grid size={{ xs: 12, sm: 6 }}>
              ...
              <FormField
                ...
                {...fieldLabelProps}
              >
                {(options?.schemeTypeOptions ?? []).map((type) => (
          ...
          <Grid container spacing={2.5}>
            ...
            <Grid size={{ xs: 12, sm: 6 }}>
              ...
              <FormField
                ...
                {...fieldLabelProps}
              >
                {(options?.rewardTypeOptions ?? []).map((type) => (
                  <MenuItem key={type} value={type}>
                    {type}
                  </MenuItem>
                ))}
            ...
            <Grid size={{ xs: 12, sm: 6 }}>
              ...
              <FormField
                ...
                {...fieldLabelProps}
              >
                {(options?.rewardFrequencyOptions ?? []).map((freq) => (
          ...
          <Grid container spacing={2.5}>
            <Grid size={{ xs: 12, sm: 6 }}>
              ...
              <FormField
                ...
                {...fieldLabelProps}
              >
                {(options?.productCategoryOptions ?? []).map((name) => (
        ...
        <Card sx={{ p: 3, mb: 3 }}>
          ...
          <Grid container spacing={2.5}>
            ...
            <Grid size={{ xs: 12, sm: 6 }}>
              <FieldLabel required={schemeCategory === 'seasonal'}>
                End Date
              </FieldLabel>
              <FormField
                name="endDate"
